...
function Confetti(_a) {
  ...
  var createConfetti = React.useCallback(function (options) {
    for (var i = 0; i < particleCount; i += 1) {
      particlesRef.current.push(new Particle(options.x, options.y, options.deg, colors, ['circle', 'square'], options.shapeSize, options.spreadDeg, launchSpeed));
    }
  }, [particleCount, colors, launchSpeed]);
  var render = React.useCallback(function () {
    ...
    animationFrameRef.current = requestAnimationFrame(frame);
  }, [x, y, deg, effectInterval, shapeSize, effectCount, spreadDeg, createConfetti]);
